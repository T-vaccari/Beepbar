...
public actor ConflictResolver {
    ...
    public func useRemote(id: UUID) async throws -> TransactionOutcome {
        try await gate.withLease(.resolving(id)) { [database, fileStore] in
            ...
            let outcome = try await coordinator.install(rootID: conflict.rootID, remoteID: conflict.remoteID, courseID: baseline?.courseID, moduleID: baseline?.moduleID, destination: conflict.relativePath, expectedLocal: expectedLocal, remote: remote, artifact: artifact)
